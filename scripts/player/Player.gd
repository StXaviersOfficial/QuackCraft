...
func _physics_process(delta: float) -> void:
        ...
        if hunger <= 0:
                ...
                health += delta * 0.5
        # Drowning
        var head_block: int = world.get_block(int(position.x), int(position.y + eye_height), int(position.z))
        if head_block == B.WATER:
                health -= delta * 1.0
        # Lava
        var foot_block: int = world.get_block(int(position.x), int(position.y), int(position.z))
